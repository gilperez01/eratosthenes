...
def sieve_slow(nmax = 55):
    """
    Sieve Algorithm Slow.
    """
    all_primes = []
    if nmax == 2: 
        all_primes = [2]
...
def sieve_fast(nmax = 55):
    """
    Sieve algorithm Fast. 
    """
    all_primes = []
    if nmax == 2: 
        all_primes = [2]
...
def get_primes(nmax=55, fast = True):
    """
    This function takes the number up until which the primes want to be computed and a boolean value for whether we want to use the slow or fast methods.
    """
    if fast:
...
def proportion_prints(all_nmax = np.arange(100, 5000, 100)):
    """
    Evaluates the proportion of prime numbers as we increase the search window
    """
    all_proportions = []
    for nmax in all_nmax:
        all_proportions.append(len(sieve_fast(nmax)/nmax))
